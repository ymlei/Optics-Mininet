...
extern int import_primitives(OpticalSwitch *optical_switch);

packet_id_t OpticalSwitch::packet_id = 0;

class OpticalSwitch::MirroringSessions {
 public:
...
class OpticalSwitch::InputBuffer {
 public:
...
OpticalSwitch::OpticalSwitch(bool enable_swap, port_t drop_port,
...
int
OpticalSwitch::receive_(port_t port_num, const char *buffer, int len) {
...
void
OpticalSwitch::start_and_return_() {
  check_queueing_metadata();

  threads_.push_back(std::thread(&OpticalSwitch::ingress_thread, this));
  for (size_t i = 0; i < nb_egress_threads; i++) {
    threads_.push_back(std::thread(&OpticalSwitch::egress_thread, this, i));
  }
  threads_.push_back(std::thread(&OpticalSwitch::transmit_thread, this));
}

void
OpticalSwitch::swap_notify_() {
  bm::Logger::get()->debug(
      "optical_switch target has been notified of a config swap");
  check_queueing_metadata();
}
...
void
OpticalSwitch::reset_target_state_() {
  bm::Logger::get()->debug("Resetting optical_switch target-specific state");
  get_component<McSimplePreLAG>()->reset_state();
}

bool
OpticalSwitch::mirroring_add_session(mirror_id_t mirror_id,
                                    const MirroringSessionConfig &config) {
  return mirroring_sessions->add_session(mirror_id, config);
}

bool
OpticalSwitch::mirroring_delete_session(mirror_id_t mirror_id) {
  return mirroring_sessions->delete_session(mirror_id);
}

bool
OpticalSwitch::mirroring_get_session(mirror_id_t mirror_id,
                                    MirroringSessionConfig *config) const {
  return mirroring_sessions->get_session(mirror_id, config);
}

int
OpticalSwitch::set_egress_priority_queue_depth(size_t port, size_t priority,
...
int
OpticalSwitch::set_egress_queue_depth(size_t port, const size_t depth_pkts) {
  egress_buffers.set_capacity(port, depth_pkts);
  return 0;
}

int
OpticalSwitch::set_all_egress_queue_depths(const size_t depth_pkts) {
  egress_buffers.set_capacity_for_all(depth_pkts);
  return 0;
}

int
OpticalSwitch::set_egress_priority_queue_rate(size_t port, size_t priority,
...
int
OpticalSwitch::set_egress_queue_rate(size_t port, const uint64_t rate_pps) {
  egress_buffers.set_rate(port, rate_pps);
  return 0;
}

int
OpticalSwitch::set_all_egress_queue_rates(const uint64_t rate_pps) {
  egress_buffers.set_rate_for_all(rate_pps);
  return 0;
}

uint64_t
OpticalSwitch::get_time_elapsed_us() const {
  return get_ts().count();
}

uint64_t
OpticalSwitch::get_time_since_epoch_us() const {
  auto tp = clock::now();
  return duration_cast<ts_res>(tp.time_since_epoch()).count();
}

void
OpticalSwitch::set_transmit_fn(TransmitFn fn) {
  my_transmit_fn = std::move(fn);
}

void
OpticalSwitch::transmit_thread() {
  while (1) {
	...
	//long ingres_time = strtol(packet->get_phv()->get_field("standard_metadata.ingress_global_timestamp").get_bytes().to_hex(false).c_str(), NULL, 16);
  //  bm::Logger::get()->error("Packet transmitted! Frame_index (64): {}, Frame_index (4): {}, Time: {}", (ingres_time >> 12) % 64, ((ingres_time >> 12) / 16) % 4, ingres_time);
  }
}

ts_res
OpticalSwitch::get_ts() const {
  return duration_cast<ts_res>(clock::now().time_since_epoch());
}

void
OpticalSwitch::enqueue(port_t egress_port, std::unique_ptr<Packet> &&packet) {
...
void
OpticalSwitch::copy_field_list_and_set_type(
...
void
OpticalSwitch::check_queueing_metadata() {
...
void
OpticalSwitch::multicast(Packet *packet, unsigned int mgid) {
...
size_t 
OpticalSwitch::ts2time_slice(int64_t current_time) {
  return ((current_time & 0x0000FFFFFFFFFFFF) >> 15) % nb_time_slices; // one slice per second
}

void
OpticalSwitch::ingress_thread() {
  PHV *phv;
  ...
  while (1) {
    ...
    //Reuse ingress_global_timestamp to represent time slice.
    size_t time_slice = ts2time_slice(get_ts().count());
    phv->get_field("standard_metadata.ingress_global_timestamp").set(time_slice);
    //phv->get_field("standard_metadata.ingress_global_timestamp")
    //      .set(get_ts().count());
		  
    //long ingres_time = strtol(packet->get_phv()->get_field("standard_metadata.ingress_global_timestamp").get_bytes().to_hex(false).c_str(), NULL, 16);
    //  bm::Logger::get()->error("Incomming packet! Meta-Time: {}, Time: {}", ingres_time, get_ts().count());
...
void
OpticalSwitch::egress_thread(size_t worker_id) {
  PHV *phv;
