...
class mark_to_drop : public ActionPrimitive<Header &> {
  void operator ()(Header &std_hdr) {
    ...
    // that alias is used in other parts of optical_switch, everything should
...
// In addition to setting the optical_switch global variable, this function also
// ensures that this unit is not discarded by the linker. It is being called by
// the constructor of OpticalSwitch.
int import_primitives(OpticalSwitch *sswitch) {
  optical_switch = sswitch;
  return 0;
}
