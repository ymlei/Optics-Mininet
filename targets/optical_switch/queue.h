...
//! thread. This is a very tor class, which does not implement anything fancy
...
class Queue {
 ...
 public:
  ...
  int pop_back(T* pItem) {
    std::unique_lock<std::mutex> lock(q_mutex);
    if (!is_not_empty()) return -1;
 ...
 private:
  ...
  bool is_not_full() const { return true;} //queue.size() > -1; }
