...
#include <bm/optical_switch/runner.h>
...
// notifications & learning notifications. optical_switch "captures" these
...
// optical_switch usually disable nanomsg anyway.
...
/* static */
constexpr uint32_t OpticalSwitchRunner::default_drop_port;

OpticalSwitchRunner::OpticalSwitchRunner(uint32_t cpu_port, uint32_t drop_port)
    : cpu_port(cpu_port),
      optical_switch(new OpticalSwitch(true /* enable_swap */, drop_port)) { }

OpticalSwitchRunner::~OpticalSwitchRunner() = default;

int OpticalSwitchRunner::init_and_start(const bm::OptionsParser &parser) {
  std::shared_ptr<bm::TransportIface> my_transport;
#ifdef WITH_PI
  my_transport = std::make_shared<NotificationsCapture>(
      optical_switch.get());
#endif
  int status = optical_switch->init_from_options_parser(
  ...
  auto transmit_fn = [this](bm::DevMgrIface::port_t port_num,
    ...
    if (cpu_port > 0 && port_num == cpu_port) {
      ...
      auto status = pi_packetin_receive(optical_switch->get_device_id(),
                                        buf, static_cast<size_t>(len));
      if (status != PI_STATUS_SUCCESS)
        bm::Logger::get()->error("Error when transmitting packet-in");
    } else {
      optical_switch->transmit_fn(port_num, buf, len);
    }
  };
  optical_switch->set_transmit_fn(transmit_fn);

  bm::pi::register_switch(optical_switch.get(), cpu_port);
#endif
...
device_id_t OpticalSwitchRunner::get_device_id() const {
  return optical_switch->get_device_id();
}

DevMgr *OpticalSwitchRunner::get_dev_mgr() {
  return optical_switch.get();
}
