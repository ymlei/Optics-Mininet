...
packet_handler(int port_num, const char *buffer, int len, void *cookie) {
  static_cast<OpticalSwitch *>(cookie)->receive(port_num, buffer, len);
}

}  // namespace

class OpticalSwitch_PacketRedirectP4 : public ::testing::Test {
 protected:
  static constexpr size_t kMaxBufSize = 512;

  static constexpr bm::device_id_t device_id{0};

  OpticalSwitch_PacketRedirectP4()
  ...
  // the optical_switch target detaches threads
  static void SetUpTestCase() {
    ...
    test_switch = new OpticalSwitch(8);  // 8 ports
    ...
    test_switch->Switch::start();  // there is a start member in OpticalSwitch
  ...
  static std::string pid(packet_id_t copy_id) {
    return std::to_string(OpticalSwitch::get_packet_id()) + "." +
 ...
 protected:
  ...
  static OpticalSwitch *test_switch;
...
const char OpticalSwitch_PacketRedirectP4::event_logger_addr[] =
    "ipc:///tmp/test_events_abc123";
const char OpticalSwitch_PacketRedirectP4::packet_in_addr[] =
    "ipc:///tmp/test_packet_in_abc123";

OpticalSwitch *OpticalSwitch_PacketRedirectP4::test_switch = nullptr;

const char OpticalSwitch_PacketRedirectP4::testdata_dir[] = TESTDATADIR;
const char OpticalSwitch_PacketRedirectP4::test_json[] =
    "packet_redirect.json";

TEST_F(OpticalSwitch_PacketRedirectP4, Baseline) {
...
struct PreTestConfiguration {
  PreTestConfiguration(McTorPreLAG *pre, int mgrp,
                       std::initializer_list<int> ports)
      : pre(pre) {
    EXPECT_EQ(McTorPreLAG::SUCCESS, pre->mc_mgrp_create(mgrp, &mgrp_hdl));
    for (auto port : ports) {
      McTorPreLAG::PortMap port_map;
      port_map[port] = true;
      McTorPreLAG::LagMap lag_map;
      node_hdls.emplace_back();
      auto &node_hdl = node_hdls.back();
      EXPECT_EQ(McTorPreLAG::SUCCESS,
  ...
  ~PreTestConfiguration() {
    for (auto node_hdl : node_hdls)
      EXPECT_EQ(McTorPreLAG::SUCCESS, pre->mc_node_destroy(node_hdl));
    EXPECT_EQ(McTorPreLAG::SUCCESS, pre->mc_mgrp_destroy(mgrp_hdl));
  }

  McTorPreLAG *pre;  // non-owning pointer
  McTorPreLAG::mgrp_hdl_t mgrp_hdl;
  std::vector<McTorPreLAG::l1_hdl_t> node_hdls;
};

}  // namespace

TEST_F(OpticalSwitch_PacketRedirectP4, Multicast) {
  static constexpr int port_in = 1;
  ...
  auto pre = test_switch->get_component<McTorPreLAG>();
  SCOPED_TRACE("OpticalSwitch_PacketRedirectP4.Multicast");
...
class OpticalSwitch_PacketRedirectP4_CloneI2E
    : public OpticalSwitch_PacketRedirectP4 {
 protected:
...
TEST_F(OpticalSwitch_PacketRedirectP4_CloneI2E, CloneI2E) {
  static constexpr int port_in = 1;
  static constexpr int port_out = 2;
...
TEST_F(OpticalSwitch_PacketRedirectP4_CloneI2E, CloneI2E_Multicast) {
  static constexpr int port_in = 1;
  static constexpr int port_out = 2;
  static constexpr int port_out_copy = 3;
  ...
  auto pre = test_switch->get_component<McTorPreLAG>();
  SCOPED_TRACE("OpticalSwitch_PacketRedirectP4_CloneI2E.CloneI2E_Multicast");
...
class OpticalSwitch_PacketRedirectP4_CloneE2E
    : public OpticalSwitch_PacketRedirectP4 {
 protected:
  void add_entries(int port_out, int mirror_id) {
    std::vector<MatchKeyParam> match_key_1;
...
TEST_F(OpticalSwitch_PacketRedirectP4_CloneE2E, CloneE2E) {
  static constexpr int port_in = 1;
  static constexpr int port_out = 2;
  static constexpr int port_out_copy = 3;
  static constexpr int mirror_id = 1;
...
TEST_F(OpticalSwitch_PacketRedirectP4_CloneE2E, CloneE2E_Multicast) {
  static constexpr int port_in = 1;
  static constexpr int port_out = 2;
  static constexpr int port_out_copy = 3;
  static constexpr int port_out_mc_copy = 4;
  ...
  auto pre = test_switch->get_component<McTorPreLAG>();
  SCOPED_TRACE("OpticalSwitch_PacketRedirectP4_CloneE2E.CloneE2E_Multicast");
  PreTestConfiguration pre_config(pre.get(), mgrp, {port_out_mc_copy});
...
TEST_F(OpticalSwitch_PacketRedirectP4, Resubmit) {
  ...
  // should be the same? Update this if this changes in optical_switch
...
TEST_F(OpticalSwitch_PacketRedirectP4, Recirculate) {
  static constexpr int port_in = 1;
  static constexpr int port_out_1 = 2;
  static constexpr int port_out_2 = 3;
...
TEST_F(OpticalSwitch_PacketRedirectP4, ExitIngress) {
  static constexpr int port_in = 1;
