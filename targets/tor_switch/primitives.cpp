...
class mark_to_drop : public ActionPrimitive<Header &> {
  void operator ()(Header &std_hdr) {
    ...
    // that alias is used in other parts of tor_switch, everything should
...
// In addition to setting the tor_switch global variable, this function also
// ensures that this unit is not discarded by the linker. It is being called by
// the constructor of TorSwitch.
int import_primitives(TorSwitch *sswitch) {
  tor_switch = sswitch;
  return 0;
}
