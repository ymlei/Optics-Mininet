...
#include <bm/tor_switch/runner.h>
...
// notifications & learning notifications. tor_switch "captures" these
...
// tor_switch usually disable nanomsg anyway.
...
/* static */
constexpr uint32_t TorSwitchRunner::default_drop_port;

TorSwitchRunner::TorSwitchRunner(uint32_t cpu_port, uint32_t drop_port)
    : cpu_port(cpu_port),
      tor_switch(new TorSwitch(true /* enable_swap */, drop_port)) { }

TorSwitchRunner::~TorSwitchRunner() = default;

int TorSwitchRunner::init_and_start(const bm::OptionsParser &parser) {
  std::shared_ptr<bm::TransportIface> my_transport;
#ifdef WITH_PI
  my_transport = std::make_shared<NotificationsCapture>(
      tor_switch.get());
#endif
  int status = tor_switch->init_from_options_parser(
  ...
  auto transmit_fn = [this](bm::DevMgrIface::port_t port_num,
    ...
    if (cpu_port > 0 && port_num == cpu_port) {
      ...
      auto status = pi_packetin_receive(tor_switch->get_device_id(),
                                        buf, static_cast<size_t>(len));
      if (status != PI_STATUS_SUCCESS)
        bm::Logger::get()->error("Error when transmitting packet-in");
    } else {
      tor_switch->transmit_fn(port_num, buf, len);
    }
  };
  tor_switch->set_transmit_fn(transmit_fn);

  bm::pi::register_switch(tor_switch.get(), cpu_port);
#endif
...
device_id_t TorSwitchRunner::get_device_id() const {
  return tor_switch->get_device_id();
}

DevMgr *TorSwitchRunner::get_dev_mgr() {
  return tor_switch.get();
}
