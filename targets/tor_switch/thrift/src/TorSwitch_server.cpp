...
class TorSwitchHandler : virtual public TorSwitchIf {
 public:
  explicit TorSwitchHandler(TorSwitch *sw)
    : switch_(sw) { }
  ...
  int32_t mirroring_mapping_add(const int32_t mirror_id,
    ...
    TorSwitch::MirroringSessionConfig config = {};  // value-initialization
  ...
  void mirroring_session_add(const int32_t mirror_id,
    ...
    TorSwitch::MirroringSessionConfig config_ = {};  // value-initialization
  ...
  void get_num_queued_packets(std::string& _return) {
    bm::Logger::get()->trace("get_num_queued_packets");
    switch_->get_num_queued_packets(_return);
  }

  void get_packet_loss_rate(std::string& _return) {
    bm::Logger::get()->trace("get_packet_loss_rate");
    switch_->get_packet_loss_rate(_return);
  }
...
stdcxx::shared_ptr<TorSwitchIf> get_handler(TorSwitch *sw) {
  return stdcxx::shared_ptr<TorSwitchHandler>(new TorSwitchHandler(sw));
}
