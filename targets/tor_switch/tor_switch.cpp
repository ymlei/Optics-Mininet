...
extern int import_primitives(TorSwitch *tor_switch);

packet_id_t TorSwitch::packet_id = 0;

class TorSwitch::MirroringSessions {
 public:
...
TorSwitch::TorSwitch(bool enable_swap, port_t drop_port,
  ...
  : Switch(enable_swap),
    ...
    egress_cq_buffers(1024, nb_calendar_queues),
    output_buffer(128),
...
int
TorSwitch::receive_(port_t port_num, const char *buffer, int len) {
  ...
  size_t time_slice = ts2time_slice(get_ts().count());
...
void
TorSwitch::start_and_return_() {
  check_queueing_metadata();

  threads_.push_back(std::thread(&TorSwitch::ingress_thread, this));
  threads_.push_back(std::thread(&TorSwitch::egress_cq_thread, this));
  for (size_t i = 0; i < nb_egress_threads; i++) {
    threads_.push_back(std::thread(&TorSwitch::egress_thread, this, i));
  }
  threads_.push_back(std::thread(&TorSwitch::transmit_thread, this));
}

void
TorSwitch::swap_notify_() {
  bm::Logger::get()->debug(
...
int
TorSwitch::set_egress_queue_depth(size_t port, const size_t depth_pkts) {
  egress_buffers.set_capacity(port, depth_pkts);
  return 0;
}

int
TorSwitch::set_all_egress_queue_depths(const size_t depth_pkts) {
  egress_buffers.set_capacity_for_all(depth_pkts);
  return 0;
}

int
TorSwitch::set_egress_priority_queue_rate(size_t port, size_t priority,
...
int
TorSwitch::set_egress_queue_rate(size_t port, const uint64_t rate_pps) {
  egress_buffers.set_rate(port, rate_pps);
  return 0;
}

int
TorSwitch::set_all_egress_queue_rates(const uint64_t rate_pps) {
  egress_buffers.set_rate_for_all(rate_pps);
  return 0;
}

uint64_t
TorSwitch::get_time_elapsed_us() const {
  return get_ts().count();
}

uint64_t
TorSwitch::get_time_since_epoch_us() const {
  auto tp = clock::now();
  return duration_cast<ts_res>(tp.time_since_epoch()).count();
}

void
TorSwitch::get_num_queued_packets(std::string& _return) const {
  std::string output;
  size_t total = 0;
  for (const auto& key : egress_cq_buffers.get_port_qs()) {
    auto q_size = egress_cq_buffers.get_q_size(key.first, key.second);
    output += "(" + std::to_string(key.first) + "," + std::to_string(key.second)
      + "): " + std::to_string(q_size) + "\n";
      total += q_size;
  }
  output += "total: " + std::to_string(total);
  
  _return = output;
}

void
TorSwitch::get_packet_loss_rate(std::string& _return) const {
  std::string output = "Received: " + std::to_string(nb_pkts_rcvd)
                       + "\nDropped: " + std::to_string(nb_pkts_dropped);
  if (nb_pkts_rcvd == 0) output += "\nRate: 0.0";
  else {
    double rate = static_cast<double>(nb_pkts_dropped) / nb_pkts_rcvd;
...
void
TorSwitch::set_transmit_fn(TransmitFn fn) {
  my_transmit_fn = std::move(fn);
}

void
TorSwitch::transmit_thread() {
  while (1) {
...
ts_res
TorSwitch::get_ts() const {
  //return duration_cast<ts_res>(clock::now() - start);
  return duration_cast<ts_res>(clock::now().time_since_epoch());
}

void
TorSwitch::enqueue_cq(port_t egress_port, size_t send_time_slice, std::unique_ptr<Packet> &&packet) {
  
  if (send_time_slice < 0 || send_time_slice >= nb_calendar_queues) {
    BMLOG_DEBUG_PKT(*packet, "ERROR: Wrong send time slice {}.", send_time_slice);
    exit(-1);
  }
  ...
  if (with_queueing_metadata) {
    ...
    phv->get_field("queueing_metadata.enq_qdepth")
        .set(egress_cq_buffers.size(egress_port, send_time_slice));
  }
  ...
  auto push_flag = egress_cq_buffers.push_front(
      egress_port, send_time_slice,
      std::move(packet));
  // push_front returns -1 if queue is full
  if (push_flag == -1) nb_pkts_dropped++;
}

void
TorSwitch::enqueue(port_t egress_port, std::unique_ptr<Packet> &&packet) {
  ...
  auto push_flag = egress_buffers.push_front(
...
void
TorSwitch::copy_field_list_and_set_type(
...
void
TorSwitch::check_queueing_metadata() {
...
void
TorSwitch::multicast(Packet *packet, unsigned int mgid) {
...
TorSwitch::ingress_thread() {
  ...
  while (1) {
    ...
    if (!phv->has_field("scalars.metadata.intermediateForward")) {
      BMLOG_DEBUG_PKT(*packet, "ERROR: No intermediateForward field.");
      exit(-1);
    }

    Field &f_intermediateForward = phv->get_field("scalars.metadata.intermediateForward");
    size_t intermediateForward = f_intermediateForward.get_uint();

    if (intermediateForward == 1) {
      // Enqueue to normal queues. To hosts.
      BMLOG_DEBUG_PKT(*packet, "Enqueue to noremal buffer port {}.", egress_port);
      enqueue(egress_port, std::move(packet));
    } else {
      // Enqueue to calendar queue.
      size_t send_time_slice = -1;
      if (phv->has_field("scalars.metadata.send_time_slice")) {
        Field &f_send_time_slice = phv->get_field("scalars.metadata.send_time_slice");
        send_time_slice = f_send_time_slice.get_uint();
      }
      BMLOG_DEBUG_PKT(*packet, "Enqueue to port {} cq {} out of {}.", egress_port, send_time_slice, nb_calendar_queues);
      enqueue_cq(egress_port, send_time_slice, std::move(packet));

      BMLOG_DEBUG("Finish enqueue to port {} cq {} out of {}.", egress_port, send_time_slice, nb_calendar_queues);

      int64_t current_time = get_ts().count();
      size_t active_q = ts2time_slice(current_time);
      BMLOG_DEBUG("Ingress: Current time {}, active q {}", current_time, active_q);
    }
  }
}

size_t 
TorSwitch::ts2time_slice(int64_t current_time) {
  return ((current_time & 0x0000FFFFFFFFFFFF) >> 15) % nb_calendar_queues; // one slice per second
}

void
TorSwitch::egress_cq_thread() {
  PHV *phv;
  ...
  while (1) {
    ...
    BMLOG_DEBUG("Current time {}, active q {}", current_time, active_q);

    while(!egress_cq_buffers.pop_back(active_q, &port, &packet)) {
      current_time = get_ts().count();
      active_q = ts2time_slice(current_time);
    }

    BMLOG_DEBUG_PKT(*packet, "Packet is popped out from q {}.",active_q);
...
void
TorSwitch::egress_thread(size_t worker_id) {
  PHV *phv;
