...
class TorSwitch : public Switch {
 public:
  ...
  static constexpr size_t default_calendar_queues_per_port = 0;
 ...
 public:
  ...
  explicit TorSwitch(bool enable_swap = false,
                        port_t drop_port = default_drop_port,
                        size_t nb_queues_per_port = default_nb_queues_per_port,
                        size_t nb_calender_queues = default_calendar_queues_per_port);
  ...
  void get_num_queued_packets(std::string& _return) const;

  void get_packet_loss_rate(std::string& _return) const;
  ...
  TorSwitch(TorSwitch &&) = delete;
  TorSwitch &&operator =(TorSwitch &&) = delete;
 ...
 private:
  void ingress_thread();
  void egress_cq_thread();
  void egress_thread(size_t worker_id);
  void transmit_thread();

  ts_res get_ts() const;
  size_t ts2time_slice(int64_t current_time);

  // TODO(antonin): switch to pass by value?
  void enqueue(port_t egress_port, std::unique_ptr<Packet> &&packet);
  void enqueue_cq(port_t egress_port, size_t send_time_slice, std::unique_ptr<Packet> &&packet);
